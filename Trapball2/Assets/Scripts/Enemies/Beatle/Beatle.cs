...
public class Beatle : MonoBehaviour, IResettable
{
    ...
    private bool isBeatleInPositionForAttack()
    {
        ...
        // Verifica si la rotaci�n est� dentro del rango de 90 +/- tolerancia o 270 +/- tolerancia
    ...
    private bool isDetectPlayer()
    {
        ...
        if (state == State.HANG)
        {
            return Mathf.Abs(distToPlayer.x) < distanceXHang // tenga un rango x para visualizarlo desde la altura.
                   && distToPlayer.y < -1 // el player est� por debajo del beatle en y.
                   && distToPlayer.y > -10; // que el player no est� a una distancia muy grande en altura.
        } else
        {
    ...
    void ManageRotation(bool force)
    {
        if (isNormalsState() || force)
        {
            ...
            transform.eulerAngles = new Vector3(0, transform.eulerAngles.y, 0); // Restricci�n en Z
